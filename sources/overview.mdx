...
Each source reads raw bytes, applies a configured [`decoding`](/sources/common/decoding) policy, and emits structured records into Tangent’s pipeline.
...
|------|--------------|----------|
| [`socket`](/sources/socket) | Local Unix domain socket (fastest for local ingestion) | `/tmp/sidecar.sock` |
| [`file`](/sources/file) | Read structured logs from local or mounted files | `/var/log/app/access.log` |
| [`msk`](/sources/msk) | Consume from Amazon MSK (Kafka) topics | `b-1.example.com:9092` |
| [`sqs`](/sources/sqs) | Poll messages from Amazon SQS queues | `https://sqs.us-east-1.amazonaws.com/...` |
...
See [Decoding](/sources/common/decoding) for format and compression options.
</ParamField>
...
* [Decoding](/sources/common/decoding)
* [Runtime Configuration](/tangent-yaml#runtime)
* [DAG](/tangent-yaml#dag)